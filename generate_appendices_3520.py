...
# --------------------------------------------------------------------------- #
HEADER_FILL = "1578BD"      # template "Sequence" header blue
HEADER_TEXT = "FFFFFF"      # white header text
BORDER_COLOR = "D9D9D9"     # light hairline inner border
TABLE_FONT = "Calibri"      # template table font
TABLE_PT = 10               # template table font size (pt)
HEADER_ROW_TWIPS = 315      # header row min height
DATA_ROW_TWIPS = 300        # data row min height
...
def _add_borders(table):
    """Template look: light hairline horizontals + inner verticals, bottom rule,
    no heavy outer box (top/left/right = none)."""
    tblPr = table._tbl.tblPr
    borders = OxmlElement("w:tblBorders")
    edges = {"top": "none", "left": "none", "right": "none",
             "bottom": "single", "insideH": "single", "insideV": "single"}
    for edge, val in edges.items():
        e = OxmlElement(f"w:{edge}")
        e.set(qn("w:val"), val)
        e.set(qn("w:sz"), "4" if val == "single" else "0")
        e.set(qn("w:space"), "0")
        e.set(qn("w:color"), BORDER_COLOR if val == "single" else "auto")
        borders.append(e)
...
def _style_run(run, *, bold=False, white=False):
...
def add_caption(doc, letter, subtitle):
    """Add a template-style 'Table X-1 <subtitle>' caption *below* the table,
    using a real SEQ field (per-appendix sequence) so it auto-numbers."""
    p = doc.add_paragraph()
    try:
    ...
    p.add_run(f"Table {letter}-")
    fld = OxmlElement("w:fldSimple")
    fld.set(qn("w:instr"), f" SEQ Table_{letter} \\* ARABIC ")
    ri = OxmlElement("w:r"); ti = OxmlElement("w:t"); ti.text = "1"; ri.append(ti); fld.append(ri)
    p._p.append(fld)
    p.add_run(f" {subtitle}")


def add_matrix(doc, letter, title, headers, data_rows, widths):
    ...
    hdr = tbl.rows[0]
    _repeat_header(hdr)
    _row_height(hdr, HEADER_ROW_TWIPS)
    for cell, text in zip(hdr.cells, headers):
        cell.text = ""
        cell.vertical_alignment = WD_ALIGN_VERTICAL.CENTER
        p = cell.paragraphs[0]; p.paragraph_format.space_after = Pt(0)
        _style_run(p.add_run(text), bold=True, white=True)
        _shade(cell, HEADER_FILL)
    for datum in data_rows:
        row = tbl.add_row()
        _row_height(row, DATA_ROW_TWIPS)
        for cell, text in zip(row.cells, datum):
            cell.text = ""
            cell.vertical_alignment = WD_ALIGN_VERTICAL.CENTER
            p = cell.paragraphs[0]; p.paragraph_format.space_after = Pt(0)
    ...
    add_caption(doc, letter, subtitle)
...
def main():
    ...
    for letter, title, headers, rows, widths in appendices:
        ...
        add_matrix(doc, letter, title, headers, rows, widths)
